...
public final class App {
  ...
  private static String ACPU ="https://www.amazon.com/gp/product/B07HHN6KBZ/ref=ox_sc_act_title_1?smid=A2Y5U0XPH5G1GE&psc=1";
  ...
  public static void main(String[] args) throws IOException {
    ...
    for (int i = 0; i < BParts.length;i++){
      document1 = Jsoup.connect(BParts[i]).userAgent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.97 Safari/537.36").timeout(0).get();
    ...
    for (int i = 0; i < IParts.length;i++){
      document2 = Jsoup.connect(IParts[i]).userAgent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.97 Safari/537.36").timeout(0).get();
    ...
    for (int i = 0; i < AParts.length;i++){
      document3 = Jsoup.connect(AParts[i]).userAgent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.97 Safari/537.36").timeout(0).get();
